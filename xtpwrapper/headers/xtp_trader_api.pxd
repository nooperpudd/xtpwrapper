...
cdef extern from "xtp_trader_api.h":
	
		# cdef cppclass TraderSpi:
		# {
		# public:
			...
			# void OnDisconnected(unsigned long long session_id, int reason) {};
			# 
			# #错误应答
			...
			# void OnOrderEvent(XTPOrderInfo *order_info, XTPRI *error_info, unsigned long long session_id) {};
			# 
			# #成交通知
			...
			# void OnTradeEvent(XTPTradeReport *trade_info, unsigned long long session_id) {};
			# 
			# #撤单出错响应
			...
			# void OnCancelOrderError(XTPOrderCancelInfo *cancel_info, XTPRI *error_info, unsigned long long session_id) {};
			# 
			# #请求查询报单响应
			...
			# void OnQueryOrder(XTPQueryOrderRsp *order_info, XTPRI *error_info, int request_id, bool is_last, unsigned long long session_id) {};
			# 
			# #请求查询成交响应
			...
			# #@param is_last 此消息响应函数是否为request_id这条请求所对应的最后一个响应，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
			# #@remark 由于支持分时段查询，一个查询请求可能对应多个响应，需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
			# void OnQueryTrade(XTPQueryTradeRsp *trade_info, XTPRI *error_info, int request_id, bool is_last, unsigned long long session_id) {};
			# 
			# #请求查询投资者持仓响应
			...
			# void OnQueryPosition(XTPQueryStkPositionRsp *position, XTPRI *error_info, int request_id, bool is_last, unsigned long long session_id) {};
			# 
			...
			# void OnQueryAsset(XTPQueryAssetRsp *asset, XTPRI *error_info, int request_id, bool is_last, unsigned long long session_id) {};
			# 
			...
			# #@param is_last 此消息响应函数是否为request_id这条请求所对应的最后一个响应，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
			# #@remark 需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
			# void OnQueryStructuredFund(XTPStructuredFundInfo *fund_info, XTPRI *error_info, int request_id, bool is_last, unsigned long long session_id) {};
			# 
			...
			# #@param is_last 此消息响应函数是否为request_id这条请求所对应的最后一个响应，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
			# #@remark 需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
			# void OnQueryFundTransfer(XTPFundTransferNotice *fund_transfer_info, XTPRI *error_info, int request_id, bool is_last, unsigned long long session_id) {};
			# 
			# #资金划拨通知
			...
			# void OnFundTransfer(XTPFundTransferNotice *fund_transfer_info, XTPRI *error_info, unsigned long long session_id) {};
			# 
			...
			# #@param is_last 此消息响应函数是否为request_id这条请求所对应的最后一个响应，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
			# #@remark 需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
			# void OnQueryETF(XTPQueryETFBaseRsp *etf_info, XTPRI *error_info, int request_id, bool is_last, unsigned long long session_id) {};
			# 
			...
			# #@param is_last 此消息响应函数是否为request_id这条请求所对应的最后一个响应，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
			# #@remark 需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
			# void OnQueryETFBasket(XTPQueryETFComponentRsp *etf_component_info, XTPRI *error_info, int request_id, bool is_last, unsigned long long session_id) {};
			# 
			...
			# #@param is_last 此消息响应函数是否为request_id这条请求所对应的最后一个响应，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
			# #@remark 需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
			# void OnQueryIPOInfoList(XTPQueryIPOTickerRsp *ipo_info, XTPRI *error_info, int request_id, bool is_last, unsigned long long session_id) {};
			# 
			...
			# #@param is_last 此消息响应函数是否为request_id这条请求所对应的最后一个响应，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
			# #@remark 需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
			# void OnQueryIPOQuotaInfo(XTPQueryIPOQuotaRsp *quota_info, XTPRI *error_info, int request_id, bool is_last, unsigned long long session_id) {};
			# 
			...
			# #@param is_last 此消息响应函数是否为request_id这条请求所对应的最后一个响应，当为最后一个的时候为true，如果为false，表示还有其他后续消息响应
			# #@remark 需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线
			# void OnQueryOptionAuctionInfo(XTPQueryOptionAuctionInfoRsp *option_info, XTPRI *error_info, int request_id, bool is_last, unsigned long long session_id) {};
...
# 	namespace API {
		...
		# public:
			...
			const char *GetTradingDay() nogil except +

			#注册回调接口
			#@param spi 派生自回调接口类的实例，请在登录之前设定
			void RegisterSpi(TraderSpi *spi) nogil except +
			...
			#获取API的发行版本号
			#@return 返回api发行版本号
			const char* GetApiVersion() nogil except +
			...
			unsigned char GetClientIDByXTPID(unsigned long long order_xtp_id) nogil except +

			#通过报单在xtp系统中的ID获取相关资金账户名
			#@return 返回资金账户名
			#@param order_xtp_id 报单在xtp系统中的ID
			#@remark 只有资金账户登录成功后,才能得到正确的信息
			const char* GetAccountByXTPID(unsigned long long order_xtp_id) nogil except +
			...
			void SetSoftwareVersion(const char* version) nogil except +

			#设置软件开发Key
			#@param key 用户开发软件Key，用户申请开户时给予，以'\0'结尾
			#@remark 此函数必须在Login之前调用
			void SetSoftwareKey(const char* key) nogil except +
			...
			unsigned long long Login(const char* ip, int port, const char* user, const char* password, XTP_PROTOCOL_TYPE sock_type) nogil except +


			#登出请求
			#@return 登出是否成功，“0”表示登出成功，“-1”表示登出失败
			#@param session_id 资金账户对应的session_id,登录时得到
			int Logout(unsigned long long session_id) nogil except +
			...
			unsigned long long InsertOrder(XTPOrderInsertInfo *order, unsigned long long session_id) nogil except +
			...
			unsigned long long CancelOrder(const unsigned long long order_xtp_id, unsigned long long session_id) nogil except +
			...
			int QueryOrderByXTPID(const unsigned long long order_xtp_id, unsigned long long session_id, int request_id) nogil except +
			...
			int QueryOrders(const XTPQueryOrderReq *query_param, unsigned long long session_id, int request_id) nogil except +
			...
			int QueryTradesByXTPID(const unsigned long long order_xtp_id, unsigned long long session_id, int request_id) nogil except +
			...
			int QueryTrades(XTPQueryTraderReq *query_param, unsigned long long session_id, int request_id) nogil except +
			...
			int QueryPosition(const char *ticker, unsigned long long session_id, int request_id) nogil except +

			#请求查询资产
			#@return 查询是否成功，“0”表示成功，非“0”表示出错，此时用户可以调用GetApiLastError()来获取错误代码
			#@param session_id 资金账户对应的session_id,登录时得到
			#@param request_id 用于用户定位查询响应的ID，由用户自定义
			int QueryAsset(unsigned long long session_id, int request_id) nogil except +
			...
			#@param request_id 用于用户定位查询响应的ID，由用户自定义
			#@remark 此函数查询出的结果可能对应多个查询结果响应
			int QueryStructuredFund(XTPQueryStructuredFundInfoReq *query_param, unsigned long long session_id, int request_id) nogil except +
			...
			#@param session_id 资金账户对应的session_id,登录时得到
			unsigned long long FundTransfer(XTPFundTransferReq *fund_transfer, unsigned long long session_id) nogil except +
			...
			#@param session_id 资金账户对应的session_id,登录时得到
			#@param request_id 用于用户定位查询响应的ID，由用户自定义
			int QueryFundTransfer(XTPQueryFundTransferLogReq *query_param, unsigned long long session_id, int request_id) nogil except +
			...
			#@param session_id 资金账户对应的session_id,登录时得到
			#@param request_id 用于用户定位查询响应的ID，由用户自定义
			int QueryETF(XTPQueryETFBaseReq *query_param, unsigned long long session_id, int request_id) nogil except +
			...
			#@param session_id 资金账户对应的session_id,登录时得到
			#@param request_id 用于用户定位查询响应的ID，由用户自定义
			int QueryETFTickerBasket(XTPQueryETFComponentReq *query_param, unsigned long long session_id, int request_id) nogil except +

			#请求查询今日新股申购信息列表
			#@return 查询是否成功，“0”表示成功，非“0”表示出错，此时用户可以调用GetApiLastError()来获取错误代码
			#@param session_id 资金账户对应的session_id,登录时得到
			#@param request_id 用于用户定位查询响应的ID，由用户自定义
			int QueryIPOInfoList(unsigned long long session_id, int request_id) nogil except +

			#请求查询用户新股申购额度信息
			#@return 查询是否成功，“0”表示成功，非“0”表示出错，此时用户可以调用GetApiLastError()来获取错误代码
			#@param session_id 资金账户对应的session_id,登录时得到
			#@param request_id 用于用户定位查询响应的ID，由用户自定义
			int QueryIPOQuotaInfo(unsigned long long session_id, int request_id) nogil except +
			...
			#@param session_id 资金账户对应的session_id,登录时得到
			#@param request_id 用于用户定位查询响应的ID，由用户自定义
			int QueryOptionAuctionInfo(XTPQueryOptionAuctionInfoReq *query_param, unsigned long long session_id, int request_id) nogil except +
